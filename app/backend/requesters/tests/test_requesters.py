...
def test_authenticate(client: FlaskClient):
    ...
    thread: Thread = Thread.query.filter_by(requester_id=requester.id).order_by(Thread.created_on.desc()).first()
    ...
    new_thread: Thread = Thread.query.filter_by(requester_id=requester.id).order_by(Thread.created_on.desc()).first()
